...
def cleanup_ddl(ddl, project_id, dataset_id):
    ...
    for line in ddl_lines:
        ...
        if "--" in line:
            continue
        if "CREATE TABLE" in line or "FOREIGN KEY" in line or "PRIMARY KEY" in line:
...
def create_bigquery_dataset(
    project_id, dataset_id, dataset_to_be_deleted, location="US"
):
    ...
    try:
        ...
        print(
            f"Dataset {dataset.dataset_id} created in project {client.project} at location {dataset.location}."
        )
    except Exception as e:
