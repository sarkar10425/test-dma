...
from data_modelling_agent.sub_agents.metadata_agent.tools import call_metadata_tool

# from data_modelling_agent.sub_agents.questionnaire_agent.tools import call_questionnaire_tool
...
def before_model_callback(
    ...
    if kind_of_activity == KIND_OF_ACTIVITY_START_FRESH:
        ...
        if kind_of_activity == None:
            print("here 1")
            if (
                last_user_message.strip() == KIND_OF_ACTIVITY_START_FRESH
                ...
                last_user_message.strip() == KIND_OF_ACTIVITY_PREVIOUS
...
root_agent = Agent(
    ...
    #     2. If user replies with 1. Then you must append these {SAMPLE_PROMPTS} sample prompts to the reponse of the user.
    #     3. If user replies with 2, then you must immediately invoke `search_agent` agent and pass this information that the user wants to continue where he left previously.
    #     **Agent Routing Guide:**
    #     - **`search_agent`**: Use for requests to search, retrieve, show existing or source data of the data model.
    #     - **`extractor_agent`**: Use for requests to extract metadata from existing/source data and data model.
    #     - **`modelling_orchestrator_agent`**: Use for requests to **design or generate** new database schemas as well as continue generating previous database , data models, or DDL scripts. This agent **creates** the DDL code.
    #         **Note**: DO **NOT** call `modelling_orchestrator_agent` unless you receive direct input from user to generate a new data model. If at any place you find a need to invoke `modelling_orchestrator_agent` agent, always ask the user before invoking.
    #     - **`ddl_agent`**: Use for requests to **execute or run** an existing DDL script to create or alter tables in the database. This agent **applies** the DDL code to BigQuery.
    ...
    #     - **`dml_agent`**: Use for requests to generate or execute SQL queries (e.g., SELECT, UPDATE), or to calculate metrics and KPIs.
    #     **Strict Rules:**
    ...
    **TASK:**
        You must follow these rules:
            ...
            3. If user replies with 1, invoke `modelling_orchestrator_agent` agent to start creating new data model
